    ...
    def get_key(self, id, batch_name,khazana_topic_name=None,khazana_url=None, verbose=True):
        ...
        try:
            ...
            if not isinstance(url_op, dict):
                debugger.error("Failed to fetch lecture URL. Check token/headers.")
                return None
            url = url_op.get('url')
            signature = url_op.get('signedUrl')
            if not url or not signature:
                debugger.error("Lecture URL response missing required fields.")
                return None
